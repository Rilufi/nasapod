...
def rtquery(hash):
    for tweet in tweepy.Cursor(api.search, q=f"{hash} -@elonmusk -elon -musk -spacex -starlink -tesla -filter:retweets -filter:replies filter:images filter:safe").items(1):
        try:
